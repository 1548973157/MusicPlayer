...
void MainWindow::setBackGround(const QString & filename){
    ...
    QSize windowSize = this->size();//获取窗口大小
    QPixmap scalePixmap = pixmap.scaled(windowSize,Qt::IgnoreAspectRatio,Qt::SmoothTransformation);//忽略比例，平滑

    QPalette palette = this->palette();
    palette.setBrush(QPalette::Window,QBrush(scalePixmap));//创建QPalette，并设置背景照片-调色板
...
/********************************选择音乐**************************************/
void MainWindow::listButton(const QString & filepath)
{
    QDir dir(filepath);
    if(!dir.exists())
    {
    ...
    for(const auto &fileInfo : fileList)
    {
        ...
        item->setSizeHint(QSize(item->sizeHint().width(), 40));// 设置每项高度为 40 像素，提升视觉体验
